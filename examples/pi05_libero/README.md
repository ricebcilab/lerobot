...
With a 3Dconnexion SpaceMouse attached (read directly from `/dev/hidraw`,
no extra install), three SpaceMouse modes are available besides the default
`mode policy` (model-only control):

- `mode teleop` — you drive the arm (SpaceMouse = x/y/z, any button toggles
  the gripper); the model is not involved.
- `mode shared_override` — pi0.5 drives, but your SpaceMouse x/y/z replaces
  the model's translation in the executed action.
- `mode shared_flow_control [tau]` — pi0.5 drives; while the stick is
  deflected, the SpaceMouse translation (normalized to the model's action
  space) is written into dims 0-2 of `x_t` for the first `tau` of the 10
  flow-matching denoising steps of each action chunk, and the remaining
  steps denoise freely. The executed action is entirely the model's output,
  steered through the early flow. Idle stick = pure policy.
